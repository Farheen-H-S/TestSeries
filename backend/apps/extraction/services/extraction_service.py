...
class ExtractionService:
    ...
    def trigger_extraction(cls, document_id: int):
        ...
        try:
            ...
            logger.error("Document not found for extraction | document_id=%s", document_id)
            return

        # Idempotency check
        if document.extraction_status in [Document.ExtractionStatus.PROCESSING, Document.ExtractionStatus.COMPLETED]:
            logger.warning(
                "Skipping document extraction: Document already in %s state | document_id=%s",
                document.extraction_status, document_id
            )
            return
        ...
        try:
            ...
            logger.exception("Failed in extraction service orchestration for document %s", document_id)
            # Update document status to FAILED
            document.extraction_status = Document.ExtractionStatus.FAILED
            document.save(update_fields=["extraction_status"])
            
            # Create extraction log record
            try:
                ...
                logger.exception("Failed to create ExtractionLog for failed orchestration: %s", log_err)
            raise
